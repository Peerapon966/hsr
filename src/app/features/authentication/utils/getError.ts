...
export function getError(props: GetErrorProps): string {
  let error: string = "";
        ...
        case "password":
          if (isInvalidPassword(props.inputValue) && props.inputValue !== "") {
            props.errorMsg.invalid
              ? (error = props.errorMsg.invalid)
              : console.error(
                  "Error message for an invalid password is missing."
                );
          }
          break;
        case "matched":
          if (
            isPasswordDoesNotMatch(props.inputValue) &&
            props.inputValue !== ""
          ) {
            props.errorMsg.invalid
              ? (error = props.errorMsg.invalid)
              : console.error(
                  "Error message for an invalid confirmation password is missing."
                );
          }
          break;
      }
  ...
  if (props.element !== null) {
    const container = props.element?.querySelector(
      ".input-container"
    ) as HTMLDivElement;
    const label = props.element?.querySelector(
      ".input-label"
    ) as HTMLLabelElement;
