...
export default function checkNumeric(e: inputEventType) {
    const eventType = e.type;
    const value = (eventType === 'paste')
    ...
    switch (eventType) {
        case 'keydown':
            ...
            if (value?.toString().search(/[0-9]/) == -1 && !allowedKeys.includes(value?.toString()) && !isCtrlKey) {
                e.preventDefault()
            }
