...
function getLocaleDateString() {
  ...
  const localTimestamp = Number(utcTimestamp.slice(utcTimestamp.indexOf(':') + 1));
...
const fileTransport = pino.transport({
  target: 'pino/file', level: process.env.LOG_LEVEL ?? 'debug', options: { colorize: true, destination: 'logs/nextjs.log', mkdir: true }
});

const consoleTransport = pino.transport({
  target: 'pino-pretty', level: 'debug', options: { colorize: true, destination: 1 }
});
...
export default class Logger {
  public static fatal: LogFn = <T extends object>(obj: T, msg?: string, ...args: any[]): void => {
    fileLogger.fatal(obj, msg!, ...args);
    consoleLogger.fatal(obj, msg!, ...args);
  }
    ;

  public static error: LogFn = <T extends object>(obj: T, msg?: string, ...args: any[]): void => {
    fileLogger.error(obj, msg, ...args);
    consoleLogger.error(obj, msg, ...args);
  }

  public static warn: LogFn = <T extends object>(obj: T, msg?: string, ...args: any[]): void => {
    fileLogger.warn(obj, msg, ...args);
    consoleLogger.warn(obj, msg, ...args);
  }

  public static info: LogFn = <T extends object>(obj: T, msg?: string, ...args: any[]): void => {
    fileLogger.info(obj, msg, ...args);
    consoleLogger.info(obj, msg, ...args);
  }

  public static debug: LogFn = <T extends object>(obj: T, msg?: string, ...args: any[]): void => {
    fileLogger.debug(obj, msg, ...args);
    consoleLogger.debug(obj, msg, ...args);
  }

  public static trace: LogFn = <T extends object>(obj: T, msg?: string, ...args: any[]): void => {
