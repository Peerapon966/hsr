...
export type UseDOMObjectProps =
  | UseDOMObjectFromIdProps
  | GetAllClassnameElementsProps
  | GetSpecificClassnameElementProps
  | UseDOMObjectFromQueryProps;

export function useDOMObject<T extends any[]>(props: UseDOMObjectProps[]): T {
  const [output, setOutput] = useState<T>(
    Array(props.length).fill(undefined) as T
  );
