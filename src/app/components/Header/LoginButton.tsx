...
const LoginModal = dynamic(() => import('src/app/features/authentication').then((module) => ({ default: module.LoginModal })), { loading: () => <h1 style={{ fontSize: '72px' }}>Loading</h1> })
...
export default function LoginButton(props: LogginButtonProps) {
  return (
    <div data-flex onClick={() => props.setShowLoginModal(true)}>
      ...
      {(props.showLoginModal) ? <LoginModal closeModal={props.closeLoginModal} /> : null}
    </div>
  )
}